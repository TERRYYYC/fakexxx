...
| [#46](https://github.com/TERRYYYC/fakexxx/issues/46)：识别 legacy `user_version=0` 非 Room DB 并安全恢复，避免首次访问崩溃；后续评论：无。 | PR #102 exact HEAD `fa346c6` open、8/8 CI 绿，且有[非作者 exact-head APPROVE](https://github.com/TERRYYYC/fakexxx/pull/102#issuecomment-5559630437)，未合入；包含 WAL/SHM 处理。可信候选 `ccef5a1` 才带后续 direct-open wiring，且同样尚未进 main。 | `ccef5a1` 的 exact-head 独立审查、direct-open 两处接线和 host/instrumentation 证据；不得把 #102 的 CI 绿冒充候选已接或 production legacy-state 已知。 | 发布 / #46：`01a076d2-2da1-7bd2-b5ea-b50539b002cb` | 保全 `ccef5a1` 候选，核验其 exact diff 后走 review；不在本表重写 Auto DB/迁移。 | 部分 `DEVICE_BLOCKED`（最终设备升级验证）；候选审查不阻塞。 |
