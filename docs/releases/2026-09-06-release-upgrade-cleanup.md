...
| [#46](https://github.com/TERRYYYC/fakexxx/issues/46)：识别 legacy `user_version=0` 非 Room DB 并安全恢复，避免首次访问崩溃；后续评论：无。 | #102 代码基点 `fa346c6` 有 8/8 CI 与[非作者 exact-head APPROVE](https://github.com/TERRYYYC/fakexxx/pull/102#issuecomment-5559630437)，且已含 checkpointed-SHM 修复。`ccef5a1` 并非 `fa346c6` 祖先（exit 1），但两者的 `AppDatabase.kt` + migration test diff 为空、`app/src` patch-id 同为 `2844742aafbdeaa3098249c0d855a23182d59af2`，是同份修复的不同提交。#102 尚无 `a645a9c` 的两处 direct-open guard；`a645a9c→fa346c6` 为 exit 1。当前 #105 `b013ff2` 则包含 `ccef5a1`（exit 0）与 `a645a9c`。 | 独立 reviewer 仅对非其所写的 `a645a9c` 两处 direct-open 接线及其在 `b013ff2` 的真实调用路径 `APPROVE`；不得把相同 SHM patch 包装成新的 review 权威，也不得扩展为完整 #105 APPROVE。#102 仍需接线集成、合入与全部原 AC；最终设备升级未验收。 | 发布 / #46：`01a076d2-2da1-7bd2-b5ea-b50539b002cb` | 将已审 `a645a9c` 等价接线纳入 #102 或明确 successor 集成候选；消费 `fa346c6` 既有非作者审查与 SHM 代码同一性，不重复制造 approval。 | 部分 `DEVICE_BLOCKED`（最终设备升级验证）；host/接线/合入闭环不阻塞。 |
