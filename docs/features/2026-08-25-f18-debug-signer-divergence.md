...
| **8. 验收** | ① 本分支两 app `assembleDebug` 产物 cert 均 `7a598cbe`（guard 通过，跨 worktree 构建亦然）② guard 对封存外来包报 `SIGNER MISMATCH` rc=2（真包负例）③ `selftest-install-apk-verified.sh` 16/16（R1 增补：真实退出码打印、exit-0-无-Success 拒绝）④ `selftest-debug-signer.sh` 7/7 ⑤ `selftest-test-hook-install-guard.sh` 8/8（R1：sed 抽取真实函数 + fake adb）⑥ CI 两 lane 各挂 `check-debug-signer.sh` gate，且三 selftest 由 `install guards (F-18 selftests)` job 在 CI 实跑（R2 接线，首跑即绿）。注：`test_cellular_acceptance_matrix.py::test_python_payload_version_is_pinned_to_writer_contract` 失败为 main 既有（main@3931c40 复核同样红），非本分支引入。|
