...
if ANDROID_HOME="$sandbox/android-sdk" \
  HOST_VERIFICATION_ADDITIONAL_TARGETS="$fixtures/host-verification-wrapped-install.sh" \
  ...
  printf 'aggregate verifier accepted a forbidden preflight target\n' >&2
  failed=$((failed + 1))
elif [ -s "$sandbox/gradle-invocations.log" ]; then
  printf 'aggregate verifier invoked a Gradle wrapper after failed preflight\n' >&2
  failed=$((failed + 1))
else
  passed=$((passed + 1))
fi
