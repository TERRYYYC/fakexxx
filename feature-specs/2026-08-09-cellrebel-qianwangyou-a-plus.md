...
> 不意味着它们消失。
>
> **这三条不是已知边界的全集。** 连同 `INV-29`（`deferred`）与 `KB-5`（契约留白）
> 的完整五条见 **§20.1**——那里是"还有哪些没被证明／没被冻结"的唯一入口。
...
| 8 | `STALE_LEASE` | 该 leaseId 对**本次操作**不可用：非本 caller 所有、已 `RELEASED`，或对 `apply`/`observe` 而言处于 `EXPIRED`/`REVOKED`/`RELEASE_INCOMPLETE`。**`release` 与 `completeAndAdvance` 例外见下** | INV-14；release 行 |
...
- §20.1 中的 `unfrozen` 条目（当前为 `KB-5`）在完成前**必须已被处置**：或经其 owner 裁定后冻结为具体规则，或被显式改判为 `limit`／`deferred` 并写明理由。**带着未冻结的契约点声称完成，等于交付一份两侧可以合法分叉的契约**；
...
**三个类别不可互相改写。** `limit` 是**已经知道证明不了**；`unfrozen` 是**还没决定**；`deferred` 两者都不是——它可触达、有确定载体，只是尚未闭合（与 §10.1 对 `deferred:<DP-x>` 的规定同源）。把留白写成上限，等于宣布它永远不修；把上限写成留白，等于承诺一件交付不出来的东西；把任一者写成 `deferred`，等于给它一个并不存在的闭合路径。
