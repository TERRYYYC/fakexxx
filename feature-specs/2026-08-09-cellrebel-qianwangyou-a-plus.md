...
新增 `N-12`（full-DAG / import）与 `N-13`（history-lost / contract）两条负例，以及 `M-5`——按 M-1 的教训，它**同时**退掉两层，因为只退一层时另一层仍会把 N-12 判红，变异会报「已绑定」却什么都没证明。

> **判据补第十四条：把多个已校验的值拼进一个字符串再解析回来时，分隔符必须被证明不可能出现在字段内部；否则对值的校验不能跨越传输存活。** 判别法：**把分隔符本身塞进每一个字段，必须红。** ——`N-12`/`N-13` 就是这条判别法本身。
...
**变异自检的真实覆盖是 5 条具名修复，不是「逐条」（v1.34 收窄 / v1.35 增补）**：`M-1`→`N-2`（第一格 exact 的**两个**实现层同时退回）、`M-2`→`P-4`（ancestry 走祖先而非只看 HEAD）、`M-3`→`N-9`（只 trim 外围）、`M-4`→`N-5`（`readonly -f each_import`）、`M-5`→`N-12`（六格传输无损，**两层同退**）。**其余负例只保证"被执行并断言退出码"，不声称 mutation-bound。** 上一版写的「逐条把修复退回去」把 4 条说成了 12 条（现为 5 条 / 14 条负例）——**声称的覆盖与能够证明的覆盖是两件事**，这正是本文 §0.1 反复记的那个形状，只不过这次长在描述 harness 的散文里。选取标准是**曾经产生过假绿的那条修复必须有变异兜底**；新增负例不自动获得变异，要么补 mutation，要么不写进覆盖声明。每条变异都**先证明真的落地**（比对变异前后 checker 的 SHA-256）——否则「用例仍通过」与「变异没应用」是同一段输出，后者会冒充前者。
