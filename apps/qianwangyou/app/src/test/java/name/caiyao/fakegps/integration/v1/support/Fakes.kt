...
class FakeQwyEnvironment(private val kv: DurableKv) : QwyEnvironment {
     ...
     * DB can genuinely hold NO schedule (§6.7.4 three-state model), and that
     * state is durable too — a memory-only "no schedule" flag would be the same
     * fake-green shape F-2 killed for the pointer. Default (key absent) = a
     * schedule exists, so no existing scenario changes behaviour.
     */
