 ...
 * stateIn subscriptions, Room/DataStore continuations resuming on real IO threads), so on
 * slow CI runners a trailing dispatch from class A lands exactly on class B's setMain.
 *
 ...
 *     scopes), so retrying converges deterministically instead of racing a fixed settle
 *     sleep. Note the library's write may take effect BEFORE its post-write check throws,
 *     so retrying set/reset is idempotent-safe.
 *
...
class MainDispatcherRule(
    ...
    /** Register any extra real scope (collectors, workers) to drain before resetMain. */
