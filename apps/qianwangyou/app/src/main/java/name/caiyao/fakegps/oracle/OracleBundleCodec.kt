...
/**
 * Strict private Bundle schema between QWY and its system-server producer.
 * Unknown, missing, or wrongly typed fields are rejected so future schema versions cannot be
 * accidentally interpreted as v1. The pure map seam is executable in host JVM tests.
 */
...
object OracleBundleCodec {
    ...
    fun decode(bundle: Bundle?): OracleWireSnapshot? {
        ...
        return try {
            decodeFields(bundle.keySet().associateWith(bundle::get))
    ...
    fun decodeFields(fields: Map<String, Any?>): OracleWireSnapshot? {
        ...
        val requiredMask = (fields[KEY_REQUIRED_COVERAGE_MASK] as? Long)?.takeIf { it >= 0L } ?: return null
        ...
        return OracleWireSnapshot(
            ...
            gpsEnabled, networkEnabled, requiredMask, installedMask, health, semanticDigest, mutationId,
        )
    }
