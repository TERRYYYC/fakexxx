...
/** Explicit system-server registrar. Binding is public; registration itself requires UID 1000. */
class OracleBridgeService : Service() {
    private val registrar = object : IContinuityOracleRegistrar.Stub() {
        override fun registerOracle(oracle: IAuthoritativeContinuityOracle?) {
            ...
            val nonNullOracle = oracle ?: throw IllegalArgumentException("continuity oracle binder is required")
            if (!OracleClientRegistry.process.register(callingUid, OracleClientRegistry.binderRegistration(nonNullOracle))) {
